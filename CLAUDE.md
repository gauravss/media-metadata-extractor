...
2. **Data collection** — uses `find` to locate audio files; for each file, runs `md5 -q` for checksum and `exiftool` with a pipe-delimited format string. Sanitization (strip `\n`/`\r`, trim/collapse whitespace) happens inline within the `exiftool -p` format string. Results are appended to a `mktemp` temp file.
